...
public:
    ...
    void setWight(unsigned int i, float val);
private:
    unsigned int size;
    float* wights;
};
...
public:
    ...
    void setMatrix(int w, int h, float wight);
    void setHidden(int i, float val);
    std::pair<int, int> getSize() { return std::pair<int, int>(in, out); };
private:
    unsigned int in; // this layer
    unsigned int out; // next layer
    Neuro* matrix; // weight
    float* hidden; // intermediate "pseudo layer" for error
